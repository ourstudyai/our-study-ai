...
export default function MobileMenu({ onClose }: MobileMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden animate-fade-in" style={{ background: 'var(--color-bg-primary)' }}>
      {/* Header */}
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `1px solid ${isActive ? 'rgba(124, 108, 240, 0.3)' : 'var(--color-border)'}`,
              }}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">{course.name}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{course.code}</p>
                </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{userProfile?.email}</p>
            </div>
          </div>
