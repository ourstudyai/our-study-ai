...
export default function AOCPanel({ courseId }: { courseId: string }) {
    ...
    const ask = async () => {
        ...
        const res = await fetch('/api/chat', {
            ...
            body: JSON.stringify({ message: question, courseId, mode: 'aoc' }),
        });
    ...
    return (
        <div className="rounded-xl p-6" style={{ background: 'var(--navy-card)', border: '1px solid var(--border)' }}>
            <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--gold)', fontFamily: 'Playfair Display, serif' }}>Areas of Concern</h2>
            <textarea
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="What areas are you struggling with?"
                rows={3}
